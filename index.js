  ...
  try {
    ...
    app.post("/success-payment", async (req, res) => {
      ...
      const { data } = await axios.get(
        `https://sandbox.sslcommerz.com/validator/api/validationserverAPI.php?val_id=${paymentSuccess.val_id}&store_id=postp68d23450a852b&store_passwd=postp68d23450a852b@ssl&format=json`
      );
      ...
      const findUser = await paymentCollection.findOne({
        transactionId: data.tran_id,
      });
      //  console.log(findUser,'find user')

      const makeMember = await userCollection.updateOne(
    ...
    app.post("/remove-comment", verifyToken, verifyAdmin, async (req, res) => {
      try {
        ...
        if (!report.reportCommentId || !report._id) {
          return res
            .status(400)
            .json({ error: "Missing reportCommentId or report _id" });
        }
    ...
    app.get("/admin-stats", verifyToken, verifyAdmin, async (req, res) => {
      ...
      const dawnVote = await postCollection
        .aggregate([
          {
            $group: {
              _id: null,
      ...
      const upVote = await postCollection
        .aggregate([
          {
            $group: {
              _id: null,
